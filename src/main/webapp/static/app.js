...
// get all the list
function fetchTodos() {
    axios.get('http://localhost:8080/v1/list/all')
        .then(response => {
            ...
            formattedTodos.sort((a, b) => new Date(b.timeStamp).getTime() - new Date(a.timeStamp).getTime());
...
function displayTodos(todos) {
    ...
    todos.forEach(todo => {
        ...
        const completedStyle = todo.status === 'COMPLETE' ? 'text-decoration: line-through;' : '';

        listItem.innerHTML = `
            <input type="checkbox" ${todo.status === 'COMPLETE' ? 'checked' : ''} onchange="updateTodoStatus(${todo.id}, this.checked)">
...
function addTodo() {
    ...
    axios.post('http://localhost:8080/v1/list/add', JSON.stringify(newTodo), {
        headers: {
...
function updateTodoStatus(id, completed) {
    ...
    axios.put(`http://localhost:8080/v1/list/edit/${id}`, updatedTodo)
        .then(response => {
            fetchTodos();
        })
        .catch(error => {
...
function deleteTodo(id) {
    axios.delete(`http://localhost:8080/v1/list/${id}`)
        .then(response => {
            fetchTodos();
        })
        .catch(error => {
